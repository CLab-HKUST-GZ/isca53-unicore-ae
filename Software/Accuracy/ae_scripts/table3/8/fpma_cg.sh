...
for cfg in "${configs[@]}"; do
  ...
  for model in "${models[@]}"; do
    echo "============================================"
    echo "Starting ${model} FPMA-CG FP8 W8A8KV16"
    echo "============================================"
    ...
    python llm_eval_wikitext_fpma.py \
      ...
      --result_method fpma_cg \
      --fp8_compensation_method fpma_cg \
    ...
    echo "Finished ${model} ${wq_datatype} wq=${wq_bits} aq=${a_bits} comp=fpma_cg"
    echo ""
  done
done
