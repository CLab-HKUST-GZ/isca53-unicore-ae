...
# Simple evaluation script for DSE-based mixed_unicore_auto_b{N} on LM-Eval tasks.
...
# Override examples:
#   BUDGET=16 ./scripts/task_acc_dse.sh
#   COUNT=1  ./scripts/task_acc_dse.sh   # use count-based variant
BUDGET=${BUDGET:-32}
...
for cfg in "${configs[@]}"; do
  ...
  for model in "${models[@]}"; do
    echo "Running DSE evaluation on LM-Eval tasks with settings:"
